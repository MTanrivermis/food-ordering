...
const Index = () => {
  ...
  const handleChange = (e, item) => {
    ...
    if (checked) {
      ...
      changePrice(-item.price);
      setExtras(extras.filter((extras) => extras.id !== item.id));
    }
  };

  return (
    <div className="flex items-center md:h-[calc(100vh_-_88px)] gap-5 py-20 flex-wrap">
      <div className="relative md:flex-1 md:h-[80%] md:w-[80%] w-36 h-36 mx-auto">
      ...
      <div className="md:flex-1 md:text-start text-center">
        ...
        <div className="flex gap-x-4 mt-6 md:justify-start justify-center">
          {extraItems.map((item) => (
            <label key={item.id} className="flex items-start gap-x-1">
              <input
                type="checkbox"
